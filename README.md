...
<p align="center">
Create a <a href="https://jj-vcs.github.io/jj/">jujutsu</a> workspace in a sibling folder, set it up and open it
</p>
...
1. Creates a workspace at `<repo>-wts/<name>`, a sibling folder. You can pass
   the name explicitly via `-n` or have it autogenerated from the parent revision's
   revision id + description, e.g. `qlvrqrmx-fix-the-login-bug`.
2. Copies any [untracked files you've opted into](#copying-untracked-files) from
   the source workspace.
3. Runs the chosen [action](#actions) in it (the default action is just to cd into
...
A script is any executable (e.g., `fish`, `bash`, `python3`, `rust-script`, binary)
...
fully interactive. It can launch an editor, a shell, `claude`, tmux — and anything it
starts runs inside the workspace.
...
The built-in `cd` moves the shell you invoked `wts` from into the new workspace.
A script can't do that directly (a child can't change its parent's directory),
but it can opt in by writing the path to `WTS_CD_FILE`, which the shell
wrapper reads:
